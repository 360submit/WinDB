...
from dataset import get_random_datasets

# Import config file
import config

# Import MSE
from spherenet_model_A import SphereMSE
...
# Train the model		
def train_model(model, optimizer, device, num_epochs=25, restore=False):
	...
	pretrain_weights_path = "./models/baseline.pth"
	...
	for epoch in range(init, num_epochs):
		...
		sal_dataloader = DataLoader(saliency_val_set, batch_size=2, shuffle=True, num_workers=0)
	
		# Iterate dataloaders
		for inputs, labels,inputs1, labels1,inputs2, labels2 in dataloader:
			# Send data to device
			inputs = inputs.to(device)
			labels = labels.to(device)
			...
			outputs,outputs1,outputs2 = model(inputs,inputs1,inputs2)
			loss0 = calc_loss(outputs, labels, metrics, 'train', model)	
			loss1 = calc_loss(outputs1, labels1, metrics, 'train', model)	
			loss2 = calc_loss(outputs2, labels1, metrics, 'train', model)	
		...
		# Append metrics
		train_loss.append(float(metrics['train_loss'] / epoch_samples/3))	
		...
		# Validation
		epoch_samples = 0
		for  inputs, labels,inputs1, labels1,inputs2, labels2 in sal_dataloader:
			inputs = inputs.to(device)
			labels = labels.to(device)
			inputs1 = inputs1.to(device)
			labels1 = labels1.to(device)
			...
			outputs,outputs1,outputs2 = model(inputs,inputs1,inputs2)
			loss0 = calc_loss(outputs, labels, metrics, 'val', model)	
			loss1 = calc_loss(outputs1, labels1, metrics, 'val', model)	
			loss2 = calc_loss(outputs2, labels1, metrics, 'val', model)	
			loss = (loss0 + loss1+loss2)/3
